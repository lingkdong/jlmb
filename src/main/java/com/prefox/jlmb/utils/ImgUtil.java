package com.prefox.jlmb.utils;

import com.prefox.jlmb.constant.Constant;
...
//					// 为等比缩放计算输出的图片宽度及高度
                ...
                ImageIO.write(tag, oldFile.substring(index + 1), outputStream);
                outputStream.close();
                return newImage;
            } catch (Exception e) {
                e.printStackTrace();
...
//					// 为等比缩放计算输出的图片宽度及高度
                ...
                /*压缩之后临时存放位置*/
                outputStream = new FileOutputStream(newImage);
                ImageIO.write(to, oldFile.substring(index + 1), outputStream);
                outputStream.close();
                return newImage;
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public static boolean isLargeThan(File file, int large) {
        try {
